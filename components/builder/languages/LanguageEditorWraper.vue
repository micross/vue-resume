...
import { defaultLanguage, type LanguageSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<LanguageSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <LanguageEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
