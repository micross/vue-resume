...
import { defaultEducation, type EducationSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<EducationSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <EducationEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
