...
const template = resumeStore.$state.resume.data.metadata
  .template as Template;
const RTemplate = getTemplate(template);
const format = resumeStore.$state.resume.data.metadata.page.format;

const style = {
  height: `calc(100vh - 64px)`,
  width: `${pageSizeMap[format].width * MM_TO_PX + 42}px`,
  gridTemplateColumns: `repeat(0, 1fr)`,
}

const convert = (a: string[][]) => a as SectionKey[][];

const metadata = resumeStore.$state.resume.data.metadata;
