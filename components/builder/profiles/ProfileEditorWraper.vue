...
import { defaultProfile, type ProfileSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<ProfileSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <ProfilesEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
