...
import { type CustomSectionSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<CustomSectionSection>>()
...
const create = () => {
    // section.items.push({ ...defaultCustomSection, id: createId() })
}
...
<template>
    <main :class="mainClass">
        ...
        <CustomSectionEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index"
