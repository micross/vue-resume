...
import { defaultPublication, type PublicationSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<PublicationSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <PublicationEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
