...
import { defaultSummary, type Section, type SummarySection } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<SummarySection>>()
...
<template>
    <main :class="mainClass">
        ...
        <SummaryEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
