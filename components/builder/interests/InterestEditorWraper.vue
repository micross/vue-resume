...
import InterestEditor from "./InterestEditor.vue";
import { createId } from "@paralleldrive/cuid2";
import { defaultInterest, type InterestSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<InterestSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <InterestEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id"/>
