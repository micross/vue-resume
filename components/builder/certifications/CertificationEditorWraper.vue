...
import { defaultCertification, type CertificationSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<CertificationSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <CertificationEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
