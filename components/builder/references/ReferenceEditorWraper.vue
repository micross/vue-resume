...
import { defaultReference, type ReferenceSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<ReferenceSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <ReferenceEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
