...
import AwardsEditor from "./AwardsEditor.vue";
import { createId } from "@paralleldrive/cuid2";
import { defaultAward, type AwardSection, type Section } from "~/lib/schema";

export type Props<T extends Section> = {
    section: T;
};

const props = defineProps<Props<AwardSection>>()
...
<template>
    <main :class="mainClass">
        ...
        <AwardsEditor v-for="(item, index) of section.items" :key="item.id" :item="item" :index="index" :section-id="props.section.id" />
