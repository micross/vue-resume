...
const Summary = ({ section }: SectionProps<SummarySection>) => {
  ...
  return (
    <section id={section.model} class="grid grid-cols-5 border-t pt-2.5">
      <div>
        <h4 class="text-base font-bold">{section.name}</h4>
      </div>
      {section.items.map((item) => (
        <div
          v-html={item.content}
          class="wysiwyg col-span-4"
...
const Section = <T,>({
  ...
  return (
    <section id={section.model} class="grid grid-cols-5 border-t pt-2.5">
      ...
      <div
        class="col-span-4 grid gap-x-6 gap-y-3"
