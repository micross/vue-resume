      ...
      {(basics: BasicsSchema) => (
        <div class="flex flex-col items-center space-y-4 text-center">
          ...
          <div class="space-y-4">
            ...
            <div class="flex flex-col items-start gap-y-1.5 rounded border border-primary px-3 py-4 text-left text-sm">
              {basics.location && (
...
const Summary = ({ section }: SectionProps<SummarySection>) => {
  ...
  return (
    <section id={section.model}>
      <h4 class="mb-2 border-b pb-0.5 text-sm font-bold">{section.name}</h4>
...
const Rating = ({ level }: RatingProps) => {
  const primaryColor =
    useResumeStore().$state.resume.data.metadata.theme.primary;
  ...
  return (
    <div class="relative">
      ...
      />
      <div
        class="absolute inset-y-0 left-0 h-2.5 w-full rounded-sm bg-primary"
...
const Link = ({ url, icon, iconOnRight, label, className }: LinkProps) => {
  ...
  return (
    <div class="flex items-center gap-x-1.5">
      {!iconOnRight &&
        (icon ?? (
          <i class="ph ph-bold ph-link text-primary group-[.sidebar]:text-primary" />
        ))}
      <a
        href={url.href}
        target="_blank"
...
const Section = <T,>({
  ...
  return (
    <section id={section.model} class="grid">
      <h4 class="mb-2 border-b pb-0.5 text-sm font-bold group-[.sidebar]:text-primary">
        {section.name}
      </h4>
...
export const Glalie = () => {
  ...
  return (
    <div class="grid min-h-[inherit] grid-cols-3">
      <div
        class="sidebar p-custom group space-y-4"
